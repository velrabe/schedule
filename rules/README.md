...
→ [`supabase/functions/_shared/rules.ts`](../supabase/functions/_shared/rules.ts) — внутри `data_model` **сначала** блок **Logical event model (READ FIRST)** (ивенты, родители, заметки, пример дня), затем остальное.
