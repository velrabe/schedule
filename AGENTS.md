...
**Текст vs скриншоты:** текст = **время, названия сессий/ивентов, merchant, счёт, привязка `(+ …)`**. Скрин = **только КБЖU (kcal/P/F/C), сумма, notes** (позиции чека, pace/км для спорта). **Никогда** не подставляй название блюда/бренда в `sessions.project` (project = только заголовок фазы, напр. «день», не «Chicken noodles»). Merchant из текста («GrabFood»), бренд с чека — в `notes`. Подробная таблица — `rules.ts` → **Text vs screenshots — TWO sources, strict split**.
